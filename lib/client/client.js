...
Client.prototype.requestAll = function (method, uri) {
  ...
  return new Promise(function(resolve, reject) {
    var errorCb, nextCb, completeCb, initPage;
    if ( typeof callbackOpt === 'function' ) {
      ...
      initPage = callbackOpt.initPage; // if we send this an observer object and that observer object has initPage set, use that for the first page we call.
    ...
    if (initPage) { // we need to add the page number to the first uri call we make if we are sent initPage.
      if (!isNaN(initPage)) { // If we are sent a pure number, add it as a parameter (translates to ?page=X)
        args[1] && args[1].push({page:initPage});
      } else { // If we are not sent a number, process initPage as the entire request url.
        args[1] = initPage;
      }
    }
    return __request.apply(self, args.concat(function (error, status, body, response, result) {
      ...
      async.whilst(
        ...
        function (err) {
          if (err) {
            return reject(errorCb(err));
          } else if(completeCb) {
            return resolve(completeCb(statusList, flatten(bodyList), responseList, resultList));
          }
        }
        );
    }));
